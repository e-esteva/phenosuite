...
def pcf(output, cell_types = None, phenotype = 'Phenotype', 
    ...
    #assert type(output) == 'list','output is a list '+str(list(pd.concat([sample['Data'][phenotype] for sample in output]).value_counts().index)) 
    ...
    #if len(cell_types) == 0:
    #    assert cell_types is None,'cell_types not found'+str(cell_types) + ' | '+str(type(cell_types))
            ...
            for cell_two in cell_types[starting_index_two:]:            
                ...
                if ((sum(selection[phenotype] == cell_one) 
                    ...
                    if cell_two == "All": #regular pcf
                        ...
                        #assert len(lambda1) == len(lambda2),str(cell_one) + '|'+str(cell_two) +' | ' +str(mapping[cell_two])+' | ' +str(len(lambda1)) +' | ' +str(len(lambda2)) +' | '+str(set(point_cloud.rx2('marks')))+' | '+str(np.sum(np.array(tuple(point_cloud.rx2('marks'))) == mapping[cell_two])) + ' | '+str(pd.DataFrame(point_cloud.rx2('marks')).value_counts())
                        ...
                        #assert len(pcf_output) < 16, str(np.sum(np.array(tuple(point_cloud.rx2('marks'))) == mapping[cell_one])) +' | '+str(np.array(tuple(point_cloud.rx2('marks'))) == mapping[cell_two])
                    ...
                    # has to be int index:
                    #assert type(pcfplot) == 'list',str(pcfplot.names) + ' | '+str(type(pcfplot.names))+' | '+str(pcfplot[list(pcfplot.names).index('iso')])
                    #pcftable=pcfplot[pcfplot.names.index('iso')]
                    ...
                    #assert len(pcf_output) < 16, str([sample[2:4] for sample in pcf_output]) + str(cell_one) + '|'+str(cell_two) +' | '+str(mapping)
...
def plot_pcf_curves(pcf_df,cell_types_,resolution,label,out_path):
  ...
  print(colors_)
  #assert len(colors_) == 0,str(dir(leg))
  for l in colors_:
    #leg.legendHandles[colors_.index(l)].set_color(colors_[colors_.index(l)])
    leg.legend_handles[colors_.index(l)].set_color(colors_[colors_.index(l)])
