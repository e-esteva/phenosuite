...
plot_scatter <- function(data, x, y, color_by, smooth = FALSE, range = c(0.01, 0.99), title = "", aspect_ratio = 1) {
  ...
  cluster_num=length(unique(data[[color_by]]))
  message(glue('cluster_num: {cluster_num}'))
  if(cluster_num > 102){
    message('generating colors')
    gradient_colors = generate_colors(cluster_num)
    discrete_colors = generate_colors(cluster_num)
    message(glue('len gradient colors: {length(gradient_colors)}'))
    message(glue('len gradient colors: {length(gradient_colors)}'))
